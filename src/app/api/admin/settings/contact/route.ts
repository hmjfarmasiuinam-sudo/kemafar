...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!body.phone || !body.email || !body.address || !body.socialMedia) {
      return NextResponse.json(
        { error: 'Invalid contact settings structure' },
        { status: 400 }
      );
    }
    ...
    if (upsertError) {
      console.error('Error upserting contact settings:', upsertError);
      return NextResponse.json(
    ...
    return NextResponse.json({ message: 'Contact settings updated successfully' });
  } catch (error) {
